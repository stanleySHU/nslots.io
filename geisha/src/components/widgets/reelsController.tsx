...
export function Reel({ dataSource, ...layerProps }: { dataSource: GameStatusModel } & ComponentProps<typeof MaskLayer>) {
    ...
    const [accTime, decTime, speed] = useMemo(() => {
        return isFastSpin ? [500, 500, 16] : [800, 800, 16];
    }, [isFastSpin]);
                                ...
                                return I_SYMBOL_XS.map((x, inColumn) => {
                                    ...
                                    return (
                                        <Layer key={`reel${inColumn}`} x={x}>
                                            <ReelController {...model} spinStatus={state.spin.status} inColumn={inColumn} startSpeed={-4} speed={speed} accTime={accTime} decTime={decTime} decEasing={Easing.Back.Out} cellCount={3} symbols={_symbols}>
                                                {
                                                    (models, blur) => {
