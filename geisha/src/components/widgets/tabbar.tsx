...
export function TabBar({ dataSource }: { dataSource: GameStatusModel }) {
    ...
    return (
        <Layer>
                ...
                <Layer l-x={585} p-x={585} l-pivot={[585, 117]} p-pivot={[585, 82]} ref={tabbar}>
                    ...
                    <Layer l-x={280} l-y={35} p-x={383} p-y={15}>
                        <Layer l-x={250} l-y={-15} p-x={202} p-y={-15}>
                            {
                                isMobile ? <Layer>
                                    <SButton y={52} interactive={isIdle && !isAutoSpin} click={setSelecterOn.bind(null, true)}>
                                        <SpriteAtlas name={"bet_amount_bg.png"} anchor={0.5} tint={isIdle && !isAutoSpin ? 0xffffff : 0x888888}></SpriteAtlas>
                                        <Label anchor={0.5} y={-2} text={toCommaAndFixed(betAmount)} maxScaleLength={140} style={{
                                    ...
                                    <Layer>
                                        ...
                                        <Layer y={52}>
                                            <Label anchor={0.5} y={-2} text={`${toCommaAndFixed(betAmount)}`} maxScaleLength={100} style={{
                                                fill: '#fcaf00', fontFamily: 'Figtree', fontWeight: '500',
                                                dropShadow: true, dropShadowColor: '#000000', dropShadowBlur: 2, dropShadowAngle: Math.PI / 6, dropShadowDistance: 2
                                            }}></Label>
                                        </Layer>
                        ...
                        <Layer x={0}>
                            ...
                            <Layer y={37}>
                                <TweenAmountLabel anchor={0.5} tmp={`${currency} {}`} p-maxScaleLength={190} l-maxScaleLength={320} text={state.user.balance.toNumber()} tweenOptions={{duration: 500}} style={{
                        ...
                        <Layer l-x={500} p-x={404}>
                            ...
                            <Layer y={37}>
                                <TweenAmountLabel anchor={0.5} tmp={`${currency} {}`} maxScaleLength={190} l-maxScaleLength={320} text={betLevel?.mul(betAmount)} style={{
                                    fontSize: 26, fill: '#eeeeee', fontFamily: 'Figtree',
                                    dropShadow: true, dropShadowColor: '#000000', dropShadowBlur: 2, dropShadowAngle: Math.PI / 6, dropShadowDistance: 2
                                }}></TweenAmountLabel>
                            </Layer>
            ...
            <Selecter on={selecterOn} onClose={setSelecterOn.bind(null, false)} betAmount={betAmount.toNumber()} lineBets={lineBets} onSelectedChange={(i) => { dispatch(action_setBetAmount(lineBets[i])) }}></Selecter>
        </Layer>
    )
}
