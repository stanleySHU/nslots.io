...
export function TabBar({ dataSource }: { dataSource: GameStatusModel }) {
    ...
    const { betLevel, betAmount } = state.spin;
        ...
        <Layer>
            <Orientation>
                <Layer l-x={585} p-x={585} l-pivot={[585, 117]} p-pivot={[585, 82]} ref={tabbar}>
                    ...
                    <SPressButton interactive={isIdle} l-x={50} l-y={73} p-x={330} p-y={-97} l-visible={true} p-visible={true} onpointerup={onMenu}>
                        <SpriteAtlas name={"menu-on.png"} anchor={0.5} tint={isIdle ? 0xffffff : 0x888888}></SpriteAtlas>
                    </SPressButton>
                    <Layer l-x={280} l-y={35} p-x={383} p-y={15}>
                        <Layer x={0}>
                            <Label anchor={0.5} text={'Balance'} style={{
                            ...
                            <Layer y={37}>
                                <TweenAmountLabel anchor={0.5} text={state.user.balance.toNumber()} style={{
                        ...
                        <Layer l-x={250} l-y={-15} p-x={202} p-y={-15}>
                            <Label l-y={5} p-y={0} anchor={0.5} text={'Bet Amount'} style={{
                                fill: '#fcaf00', fontFamily: 'Figtree', fontWeight: '500',
                                dropShadow: true, dropShadowColor: '#000000', dropShadowBlur: 2, dropShadowAngle: Math.PI / 6, dropShadowDistance: 2
                            }}></Label>
                            {
                                isMobile ? <Layer>
                                    <Button y={52} interactive={isIdle} onpointerup={setSelecterOn.bind(null, true)}>
                                        <SpriteAtlas name={"bet_amount_bg.png"} anchor={0.5} tint={isIdle ? 0xffffff : 0x888888}></SpriteAtlas>
                                    ...
                                    <Layer>
                                        ...
                                        <Layer y={53}>
                                            <SPressButton l-x={-95} p-x={-75} onpointerup={onMinus} interactive={isIdle}>
                                                <SpriteAtlas name={"bet_minus.png"} anchor={0.5} tint={isIdle ? 0xffffff : 0x888888}></SpriteAtlas>
                                            </SPressButton>
                                            <SPressButton l-x={95} p-x={75} onpointerup={onAdd} interactive={isIdle}>
                                                <SpriteAtlas name={"bet_plus.png"} anchor={0.5} tint={isIdle ? 0xffffff : 0x888888}></SpriteAtlas>
                                            </SPressButton>
                                        </Layer>
                                    </Layer>
                            }
                        ...
                        <Layer l-x={500} p-x={404}>
                            <Label anchor={0.5} text={'Total Bet'} style={{
                                fill: '#eeeeee', fontFamily: 'Figtree', fontWeight: '500',
                                dropShadow: true, dropShadowColor: '#000000', dropShadowBlur: 2, dropShadowAngle: Math.PI / 6, dropShadowDistance: 2
                            }}></Label>
                            <Layer y={37}>
                                <TweenAmountLabel anchor={0.5} text={betLevel?.mul(betAmount)} style={{
                                    fontSize: 26, fill: '#eeeeee', fontFamily: 'Figtree',
                                    dropShadow: true, dropShadowColor: '#000000', dropShadowBlur: 2, dropShadowAngle: Math.PI / 6, dropShadowDistance: 2
                                }}></TweenAmountLabel>
                            </Layer>
                ...
                <Button ref={menuP} l-visible={false} p-visible={false} onpointerup={onMenu}>
