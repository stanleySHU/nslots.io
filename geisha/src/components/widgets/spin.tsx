...
export function SpinButton({ enable = true, dataSource, ...layerProps }: { enable?: boolean, dataSource: GameStatusModel } & ComponentProps<typeof Layer>) {
    ...
    const { speedModel, betAmount, betLevel, spinModel } = state.spin;
    ...
    function onSpin(e: any = null, fromAuto: boolean = false) {
        if (isAutoSpin && !fromAuto) {
            ...
            if (gameStatus == KCurrentGameStatus.showWinAmount) {
    ...
    const winTyye = spinModel?.winType; 
    const showSpin = isSpinTweenFast || (isSpinTweenIdle && !isAutoSpin);
    const showSkin = isSpinTweenSkip && !isShowWinEffect;
    const showSkinEnable = isSpinTweenSkip && !isShowWinEffect && (gameStatus != KCurrentGameStatus.showWinAmount || (winTyye! != 'normal' && winTyye! != 'small'));
    const showAuto = isAutoSpin //&& isSpinTweenIdle;
    const spinButtonEnable = keyEnable && !isSpinTweenFast && (!isSpinTweenSkip || showSkinEnable);

    return (
        <Layer>
            <KeyPress onKeySpace={onSpin.bind(null, null, true)} enable={spinButtonEnable}></KeyPress>
            <Layer {...layerProps}>
                    ...
                    <TweenAnimationRotate speed={spinTweenStatus != 'idle' ? 2 : 0.1} >
                        {
                            (rotation) => {
                                return <Layer>
                                    <Layer visible={showAuto}>
                                        <SpriteAtlas x={98} y={78} anchor={0.5} scale={1} name={'auto.png'}></SpriteAtlas>
                                        <Label visible={isSpinTweenIdle} x={99} y={78} anchor={0.5} text={`∞`} style={{ fill: '#FFFFFF', fontSize: 55 }} />
                                    </Layer>
                                    ...
                                    <SpriteAtlas visible={showSkin} x={97} y={74} anchor={[0.49, 0.47]} name={'skip.png'} tint={showSkinEnable ? 0xffffff : 0xaaaaaa}></SpriteAtlas>
                                </Layer>
                            }
                        }
