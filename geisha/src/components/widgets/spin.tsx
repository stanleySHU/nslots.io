import { PressButton, SButton, SPressButton } from "common/components/button"
...
export function SpinButton({ enable = true, dataSource, ...layerProps }: { enable?: boolean, dataSource: GameStatusModel } & ComponentProps<typeof Layer>) {
    ...
    function onSpin(e: any = null, fromAuto: boolean = false) {
        ...
        if (isSpinTweenStop) {
            dispatch(action_spin());
        } else if (gameStatus == KCurrentGameStatus.showWinAmount2) {
            nextGameStatus();
    ...
    function onAutoSpin() {
        !isAutoSpin && isIdle && dispatch(action_setAutoSpinDetail('∞', '∞', 0, 0));
    }
    ...
    return (
        <Layer>
            ...
            <Layer {...layerProps}>
                <PressButton hitFrame={[-45, -60, 90, 90]} onpointerup={onSpin} onLongPress={onAutoSpin} interactive={spinButtonEnable}>
                    <Layer x={97} y={81} pivot={[98, 94]}>
                        ...
                        <TweenAnimationRotate speed={spinTweenStatus != 'idle' ? 2 : 0.1} >
                            {
                                (rotation) => {
                                    return <Layer>
                                        <SpriteAtlas visible={showSpin} x={1} y={-2} anchor={[0.49, 0.47]} name={'Spin.png'} rotation={rotation}></SpriteAtlas>
                                        <SpriteAtlas visible={isSpinTweenStop} x={-1} y={-6} anchor={[0.49, 0.47]} name={'stop.png'}></SpriteAtlas>
                                        <SpriteAtlas visible={showSkinEnable} x={0} y={-7} anchor={[0.49, 0.47]} name={'skip.png'}></SpriteAtlas>
                                    </Layer>
                                }
                            }
    ...
    return (
        <Orientation>
            <Layer l-x={925} l-y={-28} p-x={585} p-y={-90} l-scale={1} p-scale={1.2}>
                ...
                <SButton l-x={75} l-y={90} p-x={175} p-y={-10} click={onTurbo} l-scale={1} p-scale={0.8} interactive={isIdle && !isAutoSpin}>
