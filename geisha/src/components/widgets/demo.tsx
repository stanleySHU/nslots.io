...
export function Demo({ ...layerProps }: ComponentProps<typeof Layer>) {
    ...
    useFrame(5000, () => {
        if (!current.inScroll) setPage(1);
    }, page == 0);

    useFrame(5000, () => {
        if (!current.inScroll) setPage(2);
    }, page == 1);

    useFrame(5000, () => {
        if (!current.inScroll) setPage(3);
    }, page == 2);

    useFrame(5000, () => {
        if (!current.inScroll) setPage(0);
    }, page == 3);
    ...
    return (
        <Layer {...layerProps}>
            ...
            <MaskLayer>
                <SlideLoop selected={page} onSelectedChange={setPage} y={0} x={0} size={[400, 320]} hitFrame={[-660,-380,1100,1100]} onScrollStart={onScrollStart} onScrollEnd={onScrollEnd}>
                    <Layer>
