...
export function TipsForSpinAndBet({ dataSource, ...layerProps }: { dataSource: GameStatusModel } & ComponentProps<typeof Layer>) {
    ...
    let win = 0, existWild = false, bet = DZero;
    ...
    return (
        <Layer {...layerProps}>
            ...
            <Layer visible={isShowWinLine}>
                <BitmapText ref={text} x={0} y={30} anchor={[0.5, 0.5]} text={`Win ${toCommaAndFixed(win)}${existWild ? ' x2' : ''}`} style={{fontName: R_Yellow_Font, fontSize: 70, letterSpacing: 0}}/>
            </Layer>
        </Layer>
    )
}
