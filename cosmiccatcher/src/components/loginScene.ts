...
import { LoadingScene } from "../../engine/src/slotMachine/scenes/loadingScene";
...
export class LoginScene extends LoadingScene {
  @inject("context")
  ...
  nextScene() {
    if (this.isLoaded && this.isLogin) {
      director.sceneManager.replace("splash");
    }
  }
            ...
            if (this.netService.json) {
              if (x.value) {
                ...
                if (x.value.extraGameSettings && this.context.multiplierAsCreditBet) {
                  ...
                  this.player.multiplierValues = creditBet ? creditBet : [1];
                } else this.player.multiplierValues = [1];
